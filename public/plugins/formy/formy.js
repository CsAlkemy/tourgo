...
(function ($) {
    ...
    var x = {
        addCollapse: function (obj, cfg) {
            var def = {
                id: (function () {
                    return "i" + Math.floor(Math.random() * 1000000000).toString(36)
                })(),
        ...
        createModal: function (obj, cfg) {
            ...
            var msize = (cfg.size) ? " popup-" + cfg.size : "";
            var anim = (cfg.animated === true) ? " fade" : ""
            var mdl = $("<div>", {"class": "modal" + anim, "tabindex": "-1", "role": "dialog", "id": Id}).append(
                $("<div>", {
                    "class": "popup-dialog" + msize,
                    "role": "document"
                }).append($("<div>", {"class": "popup-content"}).append(
                    $("<div>", {"class": "popup-header"}).append($("<div>", {"class": "row"}))
                ...
                //void(0)
                $(this).find(".popup-footer .btn-primary").on("click", function () {
        ...
        createForm: function (obj, cfg, props) {
            ...
            $(obj).append($("<form>", objclas).each(function () {
                ...
                for (var i = 0; i < fm.length; i++) {
                    ...
                    if ((p.columns > 1) && (xCount % (calcColumns - 1)) == 0) {
                        $.extend(gdef, {"style": "clear:left;"});
                    }
                    ;
                    //$.extend(gdef,{"style":"background:#f00"})
                    $(this).append($("<div>", gdef).each(function () {
                        var o = $.extend(def, fm[i]);
                        $.extend(o.typeahead, fm[i].typeahead);
                        ...
                        if (o.type != "hidden") {
                            $(this).append($("<label>").html(lbl).each(function () {
                                ...
                                if (p.type == 'horizontal') {
                                    $(this).addClass('control-label').addClass('col-sm-' + factor[0])
                                }
                            }));
                        } else {
                        ...
                        switch (o.type) {
                            ...
                            case "switch":
                                ...
                                el = $("<div>", {"class": "formy-switch"})
                                    .append($("<input>", att).each(function () {
                                        $(this).val(o.value)
                                    }))
                            ...
                            case "daterange":
                                var c = o.name.split(",");
                                var end = (c.length > 1) ? c[1] : "";
                                var v = o.value.split(",");
                                var val2 = (v.length > 1) ? v[1] : "";
                        ...
                        if (p.type == 'horizontal') {
                            switch (o.type) {
                                ...
                                case "hidden":
                                    ...
                                    break;
                                case "titlebox":
                                    $(this).empty().append(o.label).each(function () {
                                        for (var key in o.attrs) {
                                            $(this).attr(key, o.attrs[key])
                                        }
                                        //$(this).append("Nice one")
                                    });
                                    break;
                                case "file":
                                    $(this).append($('<div>', {"class": "col-sm-" + factor[1]}).append($('<span>', {"class": "selection text-muted"})).prepend(' ').prepend($("<label>", {class: "btn btn-default btn-file"}).append("Browse...").append(el)));
                                ...
                                default:
                                    $(this).append($('<div>', {"class": "col-sm-" + factor[1]}).append(el));
                                    break;
                            }
                        } else {
                            switch (o.type) {
                                case "button":
                                case "submit":
                                    $(this).find('label').html('&nbsp;').css('display', 'block');
                                    $(this).append(el);
                                    break;
                                case "titlebox":
                                    $(this).empty().append(o.label).each(function () {
                                    ...
                                    break;
                                case "file":
                                    $(this).append($('<div>').append($('<span>', {"class": "selection text-muted"})).prepend(' ').prepend($("<label>", {class: "btn btn-default btn-file"}).append("Browse...").append(el)));
                                    el.on('change', function () {
                                        var input = $(this),
                                            numFiles = input.get(0).files ? input.get(0).files.length : 1,
                                            label = input.val().replace(/\\/g, '/').replace(/.*\//, '');
